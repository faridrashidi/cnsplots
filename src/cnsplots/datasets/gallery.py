...
def get_showcase_data(
    *, include_showcase_images: bool = False
    ...
    Notes
    -----
    ``iris_df`` and ``tips_df`` use the third-party CSV snapshots. Other fields
    are synthetic: random examples use a local NumPy seed of 42, and Scanpy's
    Gaussian blobs use its default seed of 0. The blob matrix is centered and
    transposed, with artificial annotations. Generated values illustrate plot
    behavior and are not measured patient or gene-expression data.

    The optional images are separate bundled assets, not generated by this
    function. Their source and permission status, along with CSV attribution
    and generation details, are documented in the distributed ``NOTICE-DATA.md``
    and the :doc:`example data guide </datasets>`.
