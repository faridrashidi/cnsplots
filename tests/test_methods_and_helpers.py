...
def test_logistic_model_paths(monkeypatch: pytest.MonkeyPatch, roc_df: pd.DataFrame, capsys: pytest.CaptureFixture[str]) -> None:
    data = pd.concat(
        [
            roc_df.rename(columns={"truth": "event", "model_a": "score", "model_b": "other"})
        ]
        * 4,
        ignore_index=True,
    )
    ...
    auc, lower, upper = model._compute_auc_ci(data["event"].values, data["score"].values, n_bootstrap=10)
    assert lower <= auc <= upper

    monkeypatch.setattr(cns.LogisticModel, "_compute_auc_ci", lambda self, y_true, y_pred_proba, n_bootstrap=1000, alpha=0.05: (0.8, 0.7, 0.9))
    model.fit()
    assert list(model.results.columns) == ["predictor", "auc", "lower_ci", "upper_ci", "hue_group"]

    data_bad = data.copy()
    data_bad.loc[data_bad["group"] == "A", "event"] = 1
    model_hue = cns.LogisticModel(data_bad, event="event", variates=["score", "missing_col"], hue="group")
    model_hue.fit()
    out = capsys.readouterr().out
    assert "Warning: No variance in outcome" in out
    assert "Error fitting missing_col" in out

    model_empty = cns.LogisticModel(data_bad[data_bad["group"] == "A"], event="event", variates=["score"], hue="group")
    model_empty.fit()
...
def test_prerank_with_mocked_gseapy(fake_gseapy_result: types.SimpleNamespace, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    res = _methods.prerank(data, {"set": ["A", "B"]}, "gene", "rank", permutation_num=10)
...
def test_prerank_import_error_prints(monkeypatch: pytest.MonkeyPatch, capsys: pytest.CaptureFixture[str]) -> None:
    ...
    with pytest.raises(UnboundLocalError):
        _methods.prerank(pd.DataFrame({"gene": ["a"], "rank": [1]}), {"set": ["A"]}, "gene", "rank")
...
def test_sankey_helpers(sankey_df: pd.DataFrame, capsys: pytest.CaptureFixture[str]) -> None:
    _sankey.check_data_matches_labels(["Start", "Middle", "End"], sankey_df["source"], "left")
    ...
    widths_left, top_edge_left = _sankey._get_positions_and_total_widths(data_frame, left_labels, "left")
    widths_right, top_edge_right = _sankey._get_positions_and_total_widths(data_frame, right_labels, "right")
...
def test_phylo_helper_functions(phylo_adata: object) -> None:
    ...
    assert _phylo._is_categorical(np.array([[1, 2], ["x", "y"]], dtype=object)) == [True, True]
